...
| Lock-free, memory ordering, atomics | C++ (`std::atomic`) | Rust | Python | You can't talk about acquire/release without it being in the language |
| RAII & Move semantics | Modern C++ (C++17/20) | Rust | C | C++ defined RAII and value categories (lvalue/rvalue) |
| Smart pointers (unique_ptr, shared_ptr) | Modern C++ | Rust | C | Ownership vs reference counting semantics |
| Polymorphism & VTable layout | C++ + ASM snippet | C (manual vtable) | Java | C++ shows _vptr memory layout and instruction dispatch overhead |
| Pointers, references, indirection | C or Modern C++ | Rust | Python, JavaScript | The concept doesn't exist in GC'd languages |
| Memory layout, struct padding, alignment | C or C++ | Rust | — | Need direct memory access and sizeof/alignof |
| Embedded MMIO & Register manipulation | C (bare-metal) | Modern C++ | Python | volatile pointers, bitwise masking, direct memory mapping |
| Microcontroller Interrupts, NVIC & Vector table | C + ARM ASM | C++ host simulation | Python | Actual hardware exception entry and stack frame save |
| RTOS Context Switch & Scheduling | ARM Cortex-M ASM + C | C++ simulation | — | PendSV context save/restore and stack swapping |
